...
permite dotar de funcionalidad extendida a las páginas Web. Actualmente el uso de este lenguaje esta tan extendido y avanzado que permite crear
	...
	\subsection{Layouts}
		...
		\begin{description}[style=unboxed,leftmargin=0cm,labelwidth=1cm]
			...
			\item[\texttt{Card}] Este layout maneja multiples \emph{componentes} hijo. Los hijos ocupan todo el espacio disponible, por lo
			  ...
			  \emph{componente}.
			\item[\texttt{Fit}] Este es un layout muy simple, este tan solo acepta un hijo que ocupa todo el espacio que el padre
			...
			\item[\texttt{HBox y VBox}] Estos son dos layouts diferentes, pero muy parecidos, razón por la que serán explicados en la
			  ...
			  de \texttt{flex} respectivamente, el primero ocupara 1/3 y el segundo 2/3 del espacio total.
		\end{description}
		\begin{figure}[h]
			\centering
...
myChart = new Highcharts.StockChart({
   ...
   series: [{
	...
	\subsection{Agrupación de datos y \emph{Candlestick}} 
		...
		\par
		El agrupamiento de datos y cálculo de estos cuatro valores para cada grupo esta implementado en el \emph{back-end}. Los servicios RPC
		...
		\emph{open} y \emph{close} son utilizados para construir el cuerpo de las velas. El cuerpo de las velas representa los datos que se
		encuentran dentro de la desviación típica. La línea que cruza el cuerpo de la vela representa el valor máximo y mínimo para cada
		grupo.
		\par
...
\section{\emph{Modelo}}
	...
	\end{itemize}
	Más allá de \texttt{Ext.data.Connection} el framework ofrece abstracciones de un nivel superior. La clase \texttt{Ext.data.Model} es una
	representación de un objeto utilizado por nuestra aplicación. Estos modelos son usados por la clase \texttt{Ext.data.Store}, que encapsula
	instancias de un modelo. La clase \texttt{Ext.data.Store} también hace uso del \texttt{Ext.data.proxy.Proxy}, abstracción que permite cargar y
...
Ext.define('MyApp.controller.Users', {
   ...
   onLaunch: function() {
	...
	\subsection{Visión general}
		...
		funciones nos muestras como estas se invocan unas a otras. Algunas funciones son invocadas al producirse un cierto evento, estos
		...
		aplicación, a excepción del evento \texttt{Ext.History.change} que es disparado al realizar un cambio en la pila de historial de
		navegación.
		\begin{figure}[h]
			\centering
		...
		\end{figure}
\section{\emph{Vista}}
	El componente de Vista en una aplicación \emph{modelo-vista-controlados}\cite{MVCWiki} es el responsable de presentar la información de la implicación
	junto a todos los componentes que componen la interfaz de usuario.  El propósito de esta sección es describir los aspectos básicos de este componente.
	\begin{figure}[h]
		\centering
	...
	\end{figure}
	\par
	La Vista en una aplicación ExtJs es compuesta por una jerarquía de \emph{componentes}. En nuestro caso en lo mas alto de esta jerarquía tenemos un
	\texttt{ViewPort} al que hemos asignado el nombre \texttt{MyViewPort}. Este es un \emph{componente} que se ajusta al área de aplicación disponible. El
	layout que determina como se posicionan los hijos de este \emph{componente} es \texttt{border}. Los componentes hijos deben especificar el atributo
	\texttt{region} que determina su posicionamiento.
	\par
	...
	\texttt{itemId} definido es \texttt{Controls}. Este \emph{contenedor} tiene tres hijos que son instancias de la misma clase. Estos tres hijos
	contienen los controles vinculados a los tres módulos funcionales. Los \texttt{itemId} que estos tres hijos especifican son:
	...
	expandidos.
	\par
	La tercer hijo del \texttt{MyViewPort} con \texttt{region:center} es también una instancia de la clase
	\texttt{Ext.container.Container}. El \texttt{ítemId} que identifica a este componente es \texttt{NavigationPanel}. El layout utilizado
	es \texttt{card} donde los hijos se solapan unos a otros y tan solo uno puede ser visible a la vez. Los \emph{componentes} hijos
	contienen todos los gráficos y tablas que nuestra aplicación ofrece, estos son mostrados en función del estado de los controles.
	\par
...
\section{Descripción funcional}
	...
	\subsection{\texttt{Spike}}
		El propósito de este módulo es ofrecer un gráfico interactivo que nos permita localizar los \emph{spikes} de forma fácil. Recordamos
		...
		línea. Por contrario si se vuelve a solicitar un intervalo que requiera agrupar los datos, el gráfico cambia a \emph{Candlestick}.
		\par
		...
		botones en la parte superior izquierda, \texttt{1m} y \texttt{All} del gráfico que nos permiten fijar un intervalo de un mes o volver
		...
		por presión o los corregidos por eficiencia. En la parte superior de gráfico son representados los valores de la presión atmosférica,
		esto nos permiten correlacionar los datos respecto a esta magnitud.
		\par
		Cuando no es necesario agrupar los datos el botón \texttt{Channel Chart} es habilitado. Este botón muestra un gráfico segundario con
		...
		gráfico nos permite observar el comportamiento de los diferentes tubos. El botón \texttt{Candle Chart} nos permite volver al gŕafico
		principal.
	\subsection{\texttt{SpikeRevised}}
		...
		datos diferente y además ofrece alguna funcionalidad extra. \texttt{Spike} utiliza los datos originales, mientras que este este módulo
		...
		utilizando el botón \texttt{Submit} que esta presente en la misma ventana. Los datos añadidos a este conjunto serán considerados como
		...
		descrita. En el gráfico será añadido un \emph{flag}, la letra que aparece en dicho \emph{flag} se corresponde con el campo
	...
	\subsection{\texttt{ChannelStats}}
		...
		identificar malfuncionamientos en canales aislados. El módulo ofrece una tabla en la que podemos inspeccionar la media, desviación
		...
		\par
		En los controles están los campos de entrada que nos permiten solicitar un intervalo de tiempo determinado. Para este intervalo serán
		calculados todos los datos y el histograma que acabamos de explicar. Inicialmente los datos y el histograma son calculados para el
		último mes. La búsqueda por intervalo esta limitada a un mes dado a que las queries necesarias para obtener estos datos son
