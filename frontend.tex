...
\section{Sencha ExtJs}
	...
	basándonos en ExtJs. En la mayoría de los casos existe un único documento HTML\cite{HTML} que contiene toda la aplicación. En este documento
...
Ext.application({
   ...
   launch: function() {
	...
	\texttt{Ext.container.Viewport}. \texttt{Viewport} es un \emph{contenedor} que representa el área de aplicación y puede haber tan sólo uno por
	aplicación. 
 	\par
	...
	un \emph{contenedor} se especifican en el campo \texttt{items} que es una lista. En el ejemplo presentado tan sólo tenemos un
	...
	\subsection{Component Manager}
		...
		En este trabajo tan sólo hemos utilizado consultas simples, razón por la que no explicamos como se realizan estas consultas avanzadas.
		\par
		...
		colisiones. 
	\subsection{Layouts}
		El propósito de esta sección es explicar los layouts utilizados en este trabajo. Tan sólo explicaremos la funcionalidad de estos sin
		...
		\begin{description}[style=unboxed,leftmargin=0cm,labelwidth=1cm]
			...
			\item[\texttt{Card}] Este layout maneja multiples \emph{componentes} hijo. Los hijos ocupan todo el espacio disponible, por lo
			  que se solapan uno a otro. Esto hace que solamente uno pueda ser visible a la vez. El layout asemeja una baraja de cartas donde
			...
			\item[\texttt{Fit}] Este es un layout muy simple, este tan sólo acepta un hijo que ocupa todo el espacio que el padre ofrece.
